...
static std::string script(const std::wstring& path,int row,const std::string& nonce){
    ...
    return "local ok,r=pcall(function() local f,e=loadfile("+q+"); assert(f,e); f(); return "+command+
...
int wmain(int argc,wchar_t** argv){
    if(argc==2&&std::wstring(argv[1])==L"--selftest"){
        ...
        if(s.find("amalur_dev.wolf()") == std::string::npos)return 3;
        if(!quote(L"C:\\bad\nname").empty())return 4;
    ...
    for(DWORD i=0;i<queuedCount;++i)if(queued[i].EventType==KEY_EVENT){
