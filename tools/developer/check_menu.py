...
io.open=function(path,mode)
 if mode=='w' then return {write=function(self,s) files[path]=s end,close=function() end} end
 if not files[path] then return nil end
 local lines={};for s in string.gmatch(files[path]..'\n','([^\n]*)\n') do lines[#lines+1]=s end
 local i=0;return {read=function() i=i+1;return lines[i] end,close=function() end}
end
...
function request(nonce,session,expiration)
 files['.\\mods\\amalur_menu_request.txt']=(session or amalur_menu_state.session)..'\n'..nonce..'\n'..tostring(expiration or now+4)
 ticks()
 return files['.\\mods\\amalur_menu_result.txt'] or ''
end
''')
