"""Opt-in bounded V3 capture. Writes diagnostic deadline only; no game inputs.
...
HEADER = packed('HEADER', [(n,U) for n in ('version','pid','drawStride','drawCapacity','published','dropped')]
    + [('until',Q)] + [(n,U) for n in ('cpuStride','cpuSlots','cpuDepth','cpuPublished')]+[('counters',U*16)])
...
COUNTERS = ('draws','deferred','missing_world','missing_skin','cpu_published','cpu_evicted','not_selected',
    'published','evicted','world_uploads','skin_uploads','pending_full','upload_budget','read_miss','cpu_dropped','upload_outside_window')


def summary(draws, cpus):
    groups=collections.defaultdict(lambda:dict(draws=0,camera_matches=0,ambiguous=0,unassociated=0,missing_cpu_candidates=0,ordinals=[]))
    ...
    return dict(draw_records=len(draws),cpu_records=len(cpus),candidate_draw_groups=dict(groups),cpu_evidence=sources,
        ...
        limitations='Rotating 24-draw windows (two frames each); 48 world and 48 skin upload reads/Present max. Missing/overwritten evidence is not a passing result. Capture overhead may affect timing. Raw scale bytes are not interpreted as unit scale.')


def self_test():
    assert (c.sizeof(Bone),c.sizeof(CPU),c.sizeof(DRAW),c.sizeof(HEADER))==(48,10708,4288,112)
    assert HEADER.until.offset==24 and DRAW.skin.offset==480
    ...
    print('PASS: V3 binary layout, control offset, explicit non-association and missing CPU evidence')
...
def capture(seconds, output):
    ...
    mapping=kernel.OpenFileMappingW(6,False,'Local\\AmalurSkinTraceV3')
    mutex=kernel.OpenMutexW(0x100001,False,'Local\\AmalurSkinTraceMutexV3')
    memory=kernel.MapViewOfFile(mapping,6,0,0,SIZE) if mapping else None
    draws=[];cpus=[];seen=set();enabled=False;missed_draws=0
    try:
        if not memory or not mutex:raise RuntimeError('No V3 mapping. This reader cannot read body021/V2.')
        ...
        try:
            ...
            if (initial.version,initial.drawStride,initial.drawCapacity,initial.cpuStride,initial.cpuSlots,initial.cpuDepth)!=(3,4288,128,10708,16,4):
...
if __name__=='__main__':
    ...
    parser.add_argument('--output',type=Path,default=Path('skin-trace-v3'))
