...
// Both stick clicks shift the left stick to the D-pad. Thumbrest touch is inert.
struct TouchInput {
...
class TouchMapper {
    ...
    static bool neutral(TouchInput t){
        return std::abs(t.leftX)<.25f&&std::abs(t.leftY)<.25f&&std::abs(t.rightX)<.25f&&std::abs(t.rightY)<.25f
            ...
            &&!t.a&&!t.b&&!t.x&&!t.y&&!t.leftClick&&!t.rightClick&&!t.menu;
    }
    ...
    void cancel(){pitchArmed_=true;ready_=attack_=previousY_=actionContext_=abilityContext_=false;heldAbilities_=0;turnArmed_=true;movementBlocked_=clicksBlocked_=previousLeftClick_=previousRightClick_=false;mapPulseUntil_=stealthPulseUntil_=leftPressTick_=0;wheelHeld_=false;}
...
public:
    ...
    MotionInputPacket map(TouchInput t,bool active,bool gameplay=true,uint64_t now=GetTickCount64()){
        ...
        const bool shift=gameplay&&chord;
        ...
        if(!gameplay)dpad(p,t.rightX,t.rightY);
            ...
            if(playMode.normal()&&pitchArmed_&&std::abs(t.rightY)>.7f){
                pitchSteps_+=t.rightY>0?1:-1;pitchArmed_=false;
            }
