...
int main(int argc,char** argv){
    ...
    while(GetTickCount64()-start<(preview?600000:cameraSweep?4000:2500)){
        ...
        if(cameraSweep&&state.paused!=0){std::fprintf(stderr,"Stopped: gameplay paused or state unavailable.\n");return 7;}
        if(initialSlot==8&&state.nativeWeaponSlot==5)drewWeapon=true;
        const char* phase=preview?"preview":cameraSweep?"camera_sweep":t<.3f?"idle":t<.7f?"attack":t<2?"hand_sweep":"recover";
        if(!observe){
            ...
            if(!preview&&!cameraSweep&&t>=.7f&&t<2){p.position[0]+=.15f*std::sin((t-.7f)*4);p.orientation[1]=std::sin(.4f*std::sin(t));p.orientation[3]=std::sqrt(1-p.orientation[1]*p.orientation[1]);}
            ...
            else if(!preview&&!cameraSweep&&t>=.3f&&t<.65f)m.buttons=XINPUT_GAMEPAD_X;input.publish(m);
        }
        if(now-lastPrint>=50){
