...
struct RecipeEnvironment {
 ...
 bool supported(const amalur::MeleeContextRecipe& r)const{return !broken&&r.owner==7&&r.equipmentGeneration==epoch&&amalur::matchesPhysicalRecipe(model,r.baseAsset,0);}
...
int main(){using namespace amalur;
 for(uint32_t model:{1520u,2478u,1250u}){
  const auto recipe=physicalMeleeRecipe(model);const auto* geometry=capturedContactProfile(model);
  check(recipe.attack&&recipe.flags==0&&geometry&&geometry->count,"supported recipe has shared geometry");
  check(recipe.hands==(model==1520?2u:1u),"single weapons only use right hand");
  check(matchesPhysicalRecipe(model,recipe.attack,0)&&!matchesPhysicalRecipe(model,recipe.attack,1),"attack flags remain exact");
 ...
 check(!currentPhysicalPublication(0,2478,0,0,100,100)&&!currentPhysicalPublication(7,1323,0,0,100,100),"missing identity and unsupported publishedmodel rejected");
 check(physicalMeleeRecipe(2478).attack==50&&physicalMeleeRecipe(1250).attack==417,"captured namespace mapping");
 for(uint32_t unsupported:{1514u,1323u,1689u,1877u,2199u,5457u,0u})check(!physicalMeleeRecipe(unsupported).attack,"unverified families remain disabled");
