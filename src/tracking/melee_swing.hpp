...
public:
    ...
    bool sample(mgs5vr::Vec3 pointRelativeToHead,uint64_t tick,unsigned generation,bool eligible,float threshold=.9f,unsigned sustainedMs=30,
        bool admit=true,float rearmSpeed=.35f,unsigned quietMs=80){
        ...
        if(speed_<rearmSpeed){
            if(!quiet_){quiet_=true;quietSince_=tick;}
            if(tick-quietSince_>=quietMs)armed_=true;
        }else quiet_=false;
        // Preparation updates motion history and quiet rearming, but never
        // consumes cooldown/direction or carries sustained-speed time into a strike.
        if(!admit){fast_=false;gate_="preparation";return false;}
        if(speed_>=threshold){
