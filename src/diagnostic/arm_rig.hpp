...
struct Scratch {amalur::RigBone bones[64];uintptr_t descriptor[3];amalur::ArmTraceRecord trace{};};
...
inline uintptr_t bodyRoot{};inline uint32_t bodyOwner{};inline unsigned bodyCenter{};
inline bool solveUnsafe(uintptr_t root,Scratch& scratch,bool solveHand=true){
    ...
    auto now=GetTickCount64();bool handValid=solveHand&&enabled.load()&&amalur::freshWeaponPose(grip,timestamp,now);
    bool leftValid=solveHand&&enabled.load()&&amalur::freshWeaponPose(leftGrip,leftTimestamp,now);
    ...
    if(firstPerson.load()&&bodyTick&&bodyTick<=now&&now-bodyTick<250){
        ...
        if(bodyRoot!=root||bodyOwner!=owner||bodyCenter!=center){
            neutralBody={};bodyRoot=root;bodyOwner=owner;bodyCenter=center;
        }
    ...
    if(!handValid&&!leftValid){
        ...
        scratch.descriptor[0]=reinterpret_cast<uintptr_t>(scratch.bones);trace.stage=8;trace.flags|=4u;++samples;return true;
    }
...
inline void resetCalibration(){AcquireSRWLockExclusive(&calibrationLock);calibratedRoot=0;neutralBody={};ReleaseSRWLockExclusive(&calibrationLock);}
inline uintptr_t trackedWeaponSlot(void* mapper,uintptr_t slot,uintptr_t output,bool solved){
    __try {
        if(!solved||(slot!=8&&slot!=9&&slot!=11)||!firstPerson.load()||!enabled.load()||output<0x34
        ...
        auto object=output-0x34;if(!weapon_control::isPlayerDaggers(object)&&!weapon_control::isSinglePlayerWeapon(object))return slot;
