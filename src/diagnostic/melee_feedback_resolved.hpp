...
inline void audioRecord(const char* route,uintptr_t manager,uintptr_t descriptor){
    ...
    __try{
        ...
        if(!allow())return;
        log("VR melee resolved audio tick=%llu trace=%u route=%s owner=%08x trackedWeapon=%08x trackedModel=%u nameValid=%d nameStatus=%s name=\"%s\" selector=%08x derived=%u index=%u stamp=%u positionBits=%08x,%08x,%08x flags=%02x,%02x,%02x,%02x\n",
...
inline uintptr_t __fastcall start(void* self,void*,uintptr_t a,uintptr_t b){
    ...
    const auto result=originalStart(self,a,b);
    if(scope.trace&&scope.kind==Kind::DerivedSound){
        __try{if(allow())log("VR melee resolved audio start tick=%llu trace=%u owner=%08x trackedWeapon=%08x trackedModel=%u success=%u instance=%u\n",
            GetTickCount64(),scope.trace,scope.owner,scope.weapon,scope.model,unsigned(result&0xff),unsigned(player_rig::word(reinterpret_cast<uintptr_t>(self)+8)));}
...
inline uintptr_t __fastcall schedule(void* self,void*,uint32_t resource,uintptr_t token,uintptr_t params){
    if(scope.trace){++scope.fxSchedules;__try{
        ...
        if(allow())log("VR melee resolved schedule tick=%llu trace=%u owner=%08x resource=%u token=%u durationRaw=%08x elapsedRaw=%08x params=%08x,%08x,%08x,%08x,%08x,%08x lifecycle=schedule-only\n",
...
inline void install(){
    ...
    log("VR melee resolved capture enabled mask=%02x expected=3f budget=128-per-second lifetimeCap=none playback=disabled lifecycle=schedule-only\n",installed);
}
}
